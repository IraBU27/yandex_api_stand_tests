...
def test_create_user_16_letter_in_first_name_get_error_response():
    negative_assert_symbol("AaaaaAaaaaaaaaaа")
# Тест 5. Успешное создание пользователя
# Параметр fisrtName-разрешены английские буквы
def test_create_user_english_letter_in_first_name_get_success_response():
    positive_assert("QWErty")
# Тест 6. Успешное создание пользователя
# Параметр fisrtName-разрешены русские буквы
...
def negative_assert_number_type_first_name(user_body):
    # В переменную user_response сохраняется результат запроса на создание пользователя:
    user_response = sender_stand_request.post_new_user(user_body)
    ...
    assert user_response.status_code == 400
# Тест 12. Ошибка
# Параметр fisrtName передан неверным типом параметра
def test_create_user_number_type_first_name_get_error_response():
    # В переменную user_body сохраняется обновлённое тело запроса
    user_body = get_user_body(12)
    # Проверка полученного ответа
    negative_assert_number_type_first_name(user_body)